...
export async function POST(request: Request) {
  try {
    ...
    const {pendingContainerIds} = body;

    if (!Array.isArray(pendingContainerIds)) {
      return NextResponse.json(
        {
          error: 'Invalid payload: pendingContainerIds array is required.',
        },
        {status: 400}
      );
    }
    ...
    const args = [pendingContainerIds];
    const tx = await contract.setPendingContainerIds(...args);
